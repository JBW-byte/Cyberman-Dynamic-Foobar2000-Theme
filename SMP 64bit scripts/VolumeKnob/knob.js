...
var TICK_COUNT = 21;
var ROTATION_OFFSET = -270; // UNCHANGED
...
function on_paint(gr){
    ...
    var len = size * 0.45;
    var thick = Math.max(1, size * 0.015);
...
function on_mouse_lbtn_down(x,y){
    dragging = hitKnob(x,y);
    if (dragging){ lastY = y; window.SetCursor(CURSOR_HAND); }
    return dragging;
}

function on_mouse_lbtn_up(){
    if (dragging){ dragging = false; window.SetCursor(CURSOR_ARROW); return true; }
    return false;
}

function on_mouse_move(x,y){
    if (!dragging) return false;
    uiVolume = Math.max(0, Math.min(100, uiVolume + (lastY - y) * CONFIG.DRAG_SCALE));
