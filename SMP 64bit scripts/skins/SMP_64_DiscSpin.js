...
window.DefineScript('SMP 64bit Disc Spin V3.9', { author: 'L.E.D.', grab_focus: true });
...
const MathX = {
	...
	// New helper to blend a color with white (used for Phosphor glow)
...
// Draw horizontal scanlines using TimerPro's high-contrast black-strip style.
...
// Draw a radial glow centered on the disc. 
// Adjusted the alpha fall-off to match TimerPro's softer "Bloom" feel.
function drawGlow(g, w, h, pc) {
	...
	// TimerPro style: use a lower multiplier (0.03) for a wider, softer bloom
	const mult  = 0.03; 
...
// Draw a glass reflection using TimerPro's Smooth-step S-Curve.
// This eliminates the "banding" effect and creates a professional gradient.
function drawReflection(g, w, h) {
	...
	for (let y = 0; y < reflH; y++) {
		const t     = 1 - (y / reflH);
		const s     = t * t * (3 - 2 * t); // TimerPro's S-Curve formula
		const alpha = Math.floor(P.opReflection * s * 0.40); // Scaled for disc aesthetics
...
// Draw phosphor effect using TimerPro's "Full-Screen Tint" style.
// Instead of lines, this creates a cohesive CRT phosphor atmosphere.
function drawPhosphor(g, w, h) {
	const themeColor = PhosphorManager.getColor();
	
	// 1. Create the a lightened version of the theme color (The Glow)
	const blended = MathX.blendWithWhite(themeColor, 0.25);
	
	// 2. Apply a subtle full-screen fill with the blended color
	// TimerPro uses opPhosphor * 0.3 for the background atmospheric glow
